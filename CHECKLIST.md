...
|---|---|---|---|---|
| K1, the display fin | The fin's CAD, sent to the print queue tonight, before it closes. One fin, for display only, a rib with a thin sheet along it | CAD ready; assigned to this agent by Ameya; print submission and person's handling check pending | `pkg/K1` | The print is submitted, and `hardware/fin/README.md` records the fin's dimensions, its material, the print settings, the CAD tool, and a screenshot or render, with the STL or STEP file beside it if it is under 5 MB |
