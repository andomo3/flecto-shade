...
The primary page (`index.html`) is farmer-selected CAD watering.
The weather and zone console is secondary at `console.html`; `replay.html` redirects there.
Each page has its own renderer and stylesheet. Both run with the static server; only the secondary console uses the optional configuration API.
